...
function validateEmail(){
    ...
    if(regexp.test(email)){ //test is predefined method to check if the entered email matches the regexp
    error = defaultMsg;}
    else {
    error = emailErrorMsg;}
    return error;
...
let regexp = /^(?=.*[A-Z])(?=.*[a-z])[A-Za-z]{8,}$/; //Regular expression to validate the password input
